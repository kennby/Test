 ...
 *🐊 ${ar6} ${anti6}*		    *🐓 ${ar12} ${anti12}*`.trim();
  ...
  if (new Date - global.db.data.users[m.sender].lastberburu < 2700000) return conn.reply(m.chat, `*🍁 Favor de descansar un momento para ir a cazar nuevamente.*\n\n⏳ Tiempo: ${clockString(time - new Date())}\n${wm}`, m);
  // conn.sendButton(m.chat, `*🍁 Favor de descansar un momento para ir a cazar nuevamente.*`, `⏳ tiempo: ${clockString(time - new Date())}\n${wm}`, null, [['😍 Animales capturados 😍', '.kandang'],[`🧰 Inventario 🧰`, `.inventario`]], m)

  setTimeout(() => {
    conn.reply(m.chat, hsl, m);
    // conn.sendButton(m.chat, hsl, wm, null, [['💖 Github 💖', '#script']], null)
    /* conn.sendHydrated(m.chat, hsl, wm, null, md, `Github`, null, null, [
[null, null]], null)*/
  }, 20000);
