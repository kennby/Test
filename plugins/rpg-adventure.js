...
const handler = async (m, {usedPrefix, conn}) => {
  try {
    ...
    if (user.health < 50) {
      return conn.reply(
          m.chat,
          `_${htki} Salud baja ${htka}_\n\n*Tu salud esta por debajo de 50!! favor de curarte con una pocion para continuar con la aventura 😘*`,
          m,
      );
    }
    if (new Date() - user.lastadventure <= cooldown) {
      return conn.reply(
          m.chat,
          `${htki} Descansando ${htka}\n\n*😘 ya aventuraste, favor de descansar o esperar a qie termine el tiempo de descanso.*\n\n*⏳Tiempo: ${timers.toTimeString()} DESCANSANDO*`,
          m,
      );
    }
...
${cmenua}`;
    ...
    conn.reply(
        m.chat,
        '*❮⛔❯➢ Ocurrio un error, por favor intenta nuevamente, tal vez la API no genero la imagen...*',
        m,
    );
  }
};
