...
const handler = async (m, {conn, args: [effect], text: txt, usedPrefix, command, name}) => {
  if (!effect) throw '*❮❤️❯➜ Por favor use el comando de esta forma.*\n\n❮👑❯➜ _#logo (efecto) (texto)_\n\n*❮🍁❯➜ Por ejemplo:*\n➜ _#logo 3d-deep-sea-metal Mitsuri_\n\n*❮❤️❯➜ Cuando aparezca que se tiene que usar dos textos, el uso es este:*\n❮👑❯➜ _#logo (efecto) (texto1|texto2)_\n\n*❮🍁❯➜ Por ejemplo:*\n➜ _#logo Wolf-Logo-Galaxy Mitsuri|Bot_\n\n\n*❮🍀 𝐋𝐈𝐒𝐓𝐀 𝐃𝐄 𝐋𝐎𝐆𝐎𝐒 𝐃𝐈𝐒𝐏𝐎𝐍𝐈𝐁𝐋𝐄𝐒 🍀❯*\n\n\n❮🎉❯➜ .logo ' + effects.map((v) => v.title).join('\n❮🎉❯➜ .logo ');
  if (!effects.find((v) => (new RegExp(v.title, 'gi')).test(effect))) throw `*❮💖❯➜ 𝐄𝐥 𝐞𝐟𝐞𝐜𝐭𝐨 ${effect} 𝐧𝐨 𝐬𝐞 𝐞𝐧𝐜𝐮𝐞𝐧𝐭𝐫𝐚 𝐞𝐧 𝐥𝐚 𝐥𝐢𝐬𝐭𝐚 𝐝𝐞 𝐥𝐨𝐠𝐨𝐬*`;  
  ...
  const res = await maker(effectoSelect?.url, [...text]).catch(_ => { throw '*❮❤️❯➜ 𝐏𝐨𝐫 𝐟𝐚𝐯𝐨𝐫, 𝐩𝐨𝐧𝐠𝐚 𝐞𝐥 𝐭𝐞𝐱𝐭𝐨 𝐝𝐞 logo*' })
   if (typeof res == 'number') throw res == -1 ? `*❮💖❯➜ 𝐄𝐥 𝐞𝐟𝐞𝐜𝐭𝐨 ${effect} 𝐧𝐨 𝐬𝐞 𝐞𝐧𝐜𝐮𝐞𝐧𝐭𝐫𝐚 𝐞𝐧 𝐥𝐚 𝐥𝐢𝐬𝐭𝐚 𝐝𝐞 𝐥𝐨𝐠𝐨𝐬*` : `*❮🍁❯➜ 𝐄𝐥 𝐮𝐬𝐨 𝐜𝐨𝐫𝐫𝐞𝐜𝐭𝐨 𝐝𝐞𝐥 𝐥𝐨𝐠𝐨, 𝐞𝐬 𝐚𝐬𝐢: ${usedPrefix + command} ${effect} ${new Array(res).fill('texto').map((v, i) => v + (i ? i + 1 : '')).join('|')}*`;
  await conn.sendMessage(m.chat, {image: {url: res.image}, caption: `\n*╔═══❖•ೋ°ৡৢ͜͡  🎆 𖥨֗°ೋ•❖═══╗*\nᬊ᭄ঔৣ͜͡ீ͜❥͜🌇 𝐋𝐎𝐆𝐎𝐒   𝐁𝐎𝐓 🌇ू ፝͜❥ᬊ᭄\n*╚═══❖•ೋ°ৡৢ͜͡  🎆 𖥨֗°ೋ•❖═══╝*\n\n😻 ★𝘼𝙦𝙪𝙞 𝙩𝙞𝙚𝙣𝙚𝙨 𝙩𝙪 𝙡𝙤𝙜𝙤, 𝙘𝙧𝙚𝙖𝙙𝙤 𝙥𝙤𝙧 𝙚𝙡 𝘽𝙤𝙩!!★\n\n✰✰✰✰✰✰✰✰✰✰✰✰✰\n\n😍 ★𝙉𝙤𝙢𝙗𝙧𝙚 𝙙𝙚𝙡 𝙚𝙛𝙚𝙘𝙩𝙤★:\n✯ *${effect}*\n\n🎆 ★𝙉𝙤𝙢𝙗𝙧𝙚 𝙙𝙚𝙡 𝙗𝙤𝙩:★\n✯ ${global.wm}\n\n🎉 ★𝙄𝙣𝙛𝙤𝙧𝙢𝙖𝙘𝙞𝙤𝙣:★\n♡︎ 𝑷𝒖𝒆𝒅𝒆𝒔 𝒉𝒂𝒄𝒆𝒓 𝒎𝒂𝒔 𝒍𝒐𝒈𝒐𝒔 𝒄𝒐𝒏 𝒎𝒂𝒔 𝒆𝒇𝒆𝒄𝒕𝒐𝒔 𝒅𝒊𝒔𝒑𝒐𝒏𝒊𝒃𝒍𝒆𝒔, 𝒑𝒆𝒓𝒔𝒐𝒏𝒂𝒍𝒊𝒛𝒂 𝒕𝒖 𝒏𝒐𝒎𝒃𝒓𝒆 𝒐 𝒂𝒍𝒈𝒖𝒏 𝒏𝒐𝒎𝒃𝒓𝒆 𝒅𝒆 𝒐𝒕𝒓𝒂 𝒑𝒆𝒓𝒔𝒐𝒏𝒂 𝒄𝒐𝒏 𝒆𝒔𝒕𝒐𝒔 𝒍𝒐𝒈𝒐𝒔 😍!`}, {quoted: m});  
};
handler.help = ['logos'];
handler.tags = ['nulis'];
