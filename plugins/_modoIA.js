...
handler.before = async (m) => {
  ...
  if (bot.modoia && !m.isGroup && !Prefijo && !m.fromMe && m.text !== '') {
     if (/^.*false|disnable|(turn)?off|0/i.test(m.text)) return;
        ...
        const sytm = await fetch(`https://raw.githubusercontent.com/egobot-supreme/Mitsuri-Bot-MD/master/src/JSON/chatgpt_indicciones.txt`).then(v => v.text());
        const sistema1 = sytm.replace('@name', namedem)
        const sistema2 = '*◊🔥➤ Tu seras "💞 Mitsuri - Bot - MD 💞", lo cual fue creado por Ego_Official y GuardianBot-MD Official.*';
        try {
