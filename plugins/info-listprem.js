...
❬😍❭➛ *Usuario:*
${userr}
${prem ? `*Tiempo Restante:*\n${clockString(usuario - new Date() * 1)}` : (isPrems ? `*Tiempo Restante:*\n*Usted es un/a usuario/a premium por ilimitado.*` : '*Usted no es un/a usuario/a premium.*')}
...
❬😍❭➛ *Usuario:*
${'@' + jid.split`@`[0]}
${premiumTime > 0 ? `*Tiempo Restante:*\n${clockString(premiumTime - new Date() * 1)}` : '*» Este no es un usuario premium.*'}`).join('')}`.trim();

  if (sortedP.filter((user) => user.premiumTime).length === 0) {
    infoprem = `*❮🔥 INFO DEL USUARIO 🔥❯:*\n\n❬😍❭➛ *Usuario:*\n${userr}\n${prem ? `*Tiempo Restante:*\n${clockString(usuario - new Date() * 1)}` : '*Usted no es un/a usuario/a premium.*'}\n\n*❬🥳 USUARIOS PREMIUM 🥳❭*\n\n🛡️ *No hay usuarios premium* 🛡️`.trim();
  }
