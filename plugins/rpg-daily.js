...
const handler = async (m, {isPrems, conn}) => {
  ...
  if (new Date - user.lastclaim < 7200000) return await conn.reply(m.chat, `❮💖❯➤ *Ya reclamaste tu regalo, por favor vuelva pronto.*\n❮⏳❯➤ Tiempo: *${msToTime(time - new Date())}* para volver a reclamar tu regalo.`, fkontak, m);
  // await conn.sendButton(m.chat, `❮💖❯➤ *Ya reclamaste tu regalo, por favor vuelva pronto.*\n❮⏳❯➤ Tiempo: *${msToTime(time - new Date())}* para volver a reclamar tu regalo`, wm, null, [['👑 Menu 👑', '/menu']], fkontak, m)
  let texto = '';
  ...
  const text = `•💖•┈┈┈••┈┈┈••✦💎✦••┈┈┈••┈┈┈•💖•
*_💞 AQUI ESTA TU REGALO USUARIO/A 💞_*
*${premium ? '😍 Recompensa Premium 🏆' : '😍 Recompensa Gratis 🎆'}*
${texto}
•💖•┈┈┈••┈┈┈••✦💎✦••┈┈┈••┈┈┈•💖•\n\n🏆 *Premium:*\n❒➢ ${premium ? '✅' : '❌'}\n${wm}`;
  const img = './Menu.png';
  await conn.sendFile(m.chat, img, 'mitsuri.jpg', text, fkontak);
  // await conn.sendButton(m.chat, text, texto + `\n\n🏆 *Premium:*\n❒➢ ${premium ? '✅' : '❌'}\n${wm}`, img, [['😘 Abrir cofre 😘', '/cofre'], ['👑 Volver al menu 👑', '/menu']], m, dos.getRandom())
