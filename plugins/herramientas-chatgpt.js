...
const handler = async (m, {conn, text, usedPrefix, command}) => {
  ...
  if (!text) throw `*❬🤖❭➤ Por favor, ingrese una peticion o una orden para esta funcion de chatgpt.*\n\n*❬😀❭➤ Ejemplos de ordenes y peticiones:*\n*➤ ${usedPrefix + command} Recomendar películas de acción en Netflix*\n*➤ ${usedPrefix + command} Codigo en JS para un juego de cartas*`;
  try {
        conn.sendPresenceUpdate('composing', m.chat);
        //let sistema1 = await fetch(`https://raw.githubusercontent.com/Skidy89/chat-gpt-jailbreak/main/Text.txt`).then(v => v.text());
        let sistema1 = `Actuaras como un Bot de WhatsApp el cual fue creado por egobot-supreme, tu seras Mitsuri - Bot - MD.`;
      ...
      try {
        conn.sendPresenceUpdate('composing', m.chat);
        const syms1 = `Actuaras como un Bot de WhatsApp el cual fue creado por egobot-supreme, tu seras Mitsuri - Bot - MD.`;
      ...
      try {
        ...
        throw `*⚠️ ERROR, INTENTÉ NUEVAMENTE ⚠️*`;
     }}
    }}
   }}
  }}
